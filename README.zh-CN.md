...
`unique = "email"` 支持 Vec、数组和切片引用，不要求 `Member` 自身实现 `Value`；被投影的 `email` 字段必须实现 `Value`。重复时错误位于 `Team.members`，并保留 `field = "email"` 参数。Map、嵌套路径和原生 `Validator::value(&members, "unique=email")` 不支持字段投影；单值入口会返回 `MissingFieldContext`。
...
元素字段错误使用 `users[0].email` namespace；包括 `null` 在内的非对象元素使用 `users[0]` namespace 返回 `type` 错误。如果 `unique: email` 投影到的输入不符合子字段类型，unique 会跳过该投影，由子字段返回带索引的 `type` 错误；错误输入不会被误判成规则配置错误。
